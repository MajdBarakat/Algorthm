...
export function clearPath() {
  const cells = document.querySelectorAll('[data-state = "path"]');
  cells.forEach((cell) => {
    cell.dataset.state = "unvisited";
  });
  const cells2 = document.querySelectorAll('[data-state = "visited"]');
  cells2.forEach((cell) => {
    cell.dataset.state = "unvisited";
  });
}

export async function depthFirstSearch(rows, cols, speed) {
  clearPath();
  let startCell;
  try {
    let x = document.querySelector(`[data-state = "start"]`).dataset.position;
  } catch (err) {
    alert("Please pick a start node!");
  }
  startCell = document.querySelector(`[data-state = "start"]`);
  ...
  while (historyStack.length > 0) {
    ...
    var availableCells = checkIfAvailable2(
      checkNeighbours(
    ...
    if (availableCells.length == 0) {
      ...
      for (let i = 0; i < historyStack.length; i++) {
        ...
        i > 0 ? drawPath(historyStack[i], "path") : false;
      }
      console.log("end");
      break;
    } else {
...
export async function dijkstra(rows, cols, speed) {
  clearPath();
  let startCell;
  var unvisited = document.querySelectorAll(
    '[data-state = "unvisited"]'
  ).length;
  try {
    startCell = document
      .querySelector(`[data-state = "start"]`)
      .dataset.position.split(",")
      .map(Number);
  } catch (err) {
    alert("Please pick a start node!");
  }
  ...
  while (unvisited > 0) {
    ...
    for (let i = 0; i < currentCells.length; i++) {
      ...
      await new Promise((resolve) => setTimeout(resolve, speed));
      targetCell.dataset.state != "start"
        ? (targetCell.dataset.state = "visited")
        : false;
      targetCell.setAttribute("data-dist", `${dist}`);
      var availableNeighbours = checkIfAvailable2(
        checkNeighbours(currentCells[i], rows, cols, 1)
      );
      if (availableNeighbours != false || availableNeighbours.length == 0) {
        ...
        for (let i = 0; i < finalDist + 1; i++) {
          var pathNeighbour = checkIfDist(
            checkIfType(
              checkNeighbours(
                nextCell.dataset.position.split(",").map(Number),
                rows,
                cols,
                1
              ),
          ...
          nextCell = document.querySelector(
            `[data-position = "${chooseRandomNeighbour(pathNeighbour)}"]`
          );
          await new Promise((resolve) => setTimeout(resolve, speed));
        }
        return (unvisited = 0);
...
export async function aStar(rows, cols, speed) {
  clearPath();
  var unvisited = document.querySelectorAll(
    '[data-state = "unvisited"]'
  ).length;
  ...
  try {
    startCell = document
      ...
      .map(Number);
    endCell = document
      .querySelector(`[data-state = "end"]`)
      .dataset.position.split(",")
      .map(Number);
  } catch (err) {
    return alert("Please pick a start and end nodes!");
  }
  ...
  distanceToFinish[0] > distanceToFinish[1]
    ? (direction = "vertical")
  ...
  while (unvisited > 0) {
    ...
    for (let i = 0; i < currentCells.length; i++) {
      ...
      distanceToFinish = [
        endCell[0] - currentCells[i][0],
        endCell[1] - currentCells[i][1],
      ];
      ...
      const distanceFromStart =
        Math.abs(startCell[0] - currentCells[i][0]) +
        Math.abs(startCell[1] - currentCells[i][1]);
      if (availableNeighbours != false || availableNeighbours.length == 0) {
        for (let j = 0; j < availableNeighbours.length; j++) {
          if (
            direction == "horizontal" &&
            availableNeighbours[j][0] == currentCells[i][0] &&
            findDistance(availableNeighbours[j], endCell) <
              findDistance(currentCells[i], endCell)
          ) {
            available.push(availableNeighbours[j]);
          } else if (
            direction == "vertical" &&
            availableNeighbours[j][1] == currentCells[i][1] &&
            findDistance(availableNeighbours[j], endCell) <
              findDistance(currentCells[i], endCell)
          ) {
            available.push(availableNeighbours[j]);
        ...
        dijkstraCalcDist(startCell, rows, cols);
        const finalDist = parseInt(targetCell.dataset.dist);
        var nextCell = targetCell;
        for (let i = 0; i < finalDist + 1; i++) {
          var pathNeighbour = checkIfDist(
    ...
    if (available.length == 0) {
      ...
      while (neighbourFound == false) {
        ...
        const n = checkIfAvailable2(
          checkNeighbours(historyStack[index][0], cols, rows, 1)
        );
        if (n.length > 0) {
          currentCells = [n[0]];
